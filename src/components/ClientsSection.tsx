    ...
    <section className="py-12 bg-[#0B0F19] relative border-t border-white/5 overflow-hidden">
      ...
      <div className="section-container relative z-10 px-6">
        <div className="flex flex-col items-center mb-10 space-y-2">
          ...
          >
            <span className="text-[11px] font-mono text-yellow-400 uppercase tracking-[0.3em] font-bold">Trusted Network</span>
          </motion.div>
          <motion.h2
        ...
        <div className="space-y-6 relative">
          ...
          <div className="absolute left-0 top-0 w-32 md:w-60 h-full bg-gradient-to-r from-[#0B0F19] to-transparent z-20 pointer-events-none" />
          <div className="absolute right-0 top-0 w-32 md:w-60 h-full bg-gradient-to-l from-[#0B0F19] to-transparent z-20 pointer-events-none" />
          ...
          <div className="relative overflow-hidden py-2">
            ...
            >
              {[...clientsRow1, ...clientsRow1].map((client, i) => (
                <div 
                  key={`${client.name}-${i}`}
                  className="group relative flex items-center justify-center w-40 h-20 md:w-64 md:h-32 bg-white/[0.03] border border-white/10 rounded-2xl md:rounded-[2.5rem] hover:bg-white/10 hover:border-yellow-400/50 transition-all duration-500 overflow-hidden p-2"
                >
                  <img 
                    src={client.img} 
                    alt={client.name} 
        ...
        <div className="mt-10 flex flex-col md:flex-row items-center justify-center gap-6 opacity-40 text-xs font-mono text-zinc-300 uppercase tracking-[0.4em]">
           <span className="flex items-center gap-2"><div className="w-1 h-1 bg-yellow-400 rounded-full" /> 24/7 NETWORK OPERATIONAL</span>
           <span className="hidden md:block">|</span>
           <span className="flex items-center gap-2"><div className="w-1 h-1 bg-white rounded-full" /> SCALE: INFINITE_GROWTH</span>
        </div>
      </div>
    </section>
  );
